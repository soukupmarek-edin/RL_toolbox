...
def sarsa_eval(env, config, q_table, max_steps):
    """
    ...
    """
    eval_agent = SarsaAgent(action_space=env.action_space, obs_space=env.observation_space,
...
def train(env, config):
    agent = SarsaAgent(action_space=env.action_space, obs_space=env.observation_space,
    ...
    for eps_num in range(total_eps):
        ...
        if eps_num > 0 and eps_num % config["eval_freq"] == 0:
            ...
            mean_return, negative_returns = sarsa_eval(env, config, agent.q_table, MAX_EVAL_EPS_STEPS)
...
if __name__ == "__main__":
    ...
    # ax1.set_title("Sarsa training and evaluation")
